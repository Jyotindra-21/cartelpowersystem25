...
export function TeamMemberManagement({ initialTeamMembers = [] }: TeamMemberManagementProps) {
    const router = useRouter()
    const columns = getColumns()
    const [activeTab, setActiveTab] = useState<'all' | 'published' | 'hidden'>('all')
    const [isRefreshing, setIsRefreshing] = useState(false)
    
    // Filter teamMembers based on active tab
    const filteredTeamMembers = initialTeamMembers.filter(teamMember => {
        if (activeTab === 'all') return true
        if (activeTab === 'published') return teamMember.status
        if (activeTab === 'hidden') return !teamMember.status
        return true
    })
    ...
    const teamMemberCounts = {
        all: initialTeamMembers.length,
        published: initialTeamMembers.filter(t => t.status).length,
        hidden: initialTeamMembers.filter(t => !t.status).length,
    }
            ...
            <div className="flex flex-col md:flex-row md:items-center justify-between gap-4">
                ...
                <div className="flex items-center gap-2 justify-end">
                    ...
                    <Tooltip>
                        ...
                        <TooltipContent>Add New Team Member</TooltipContent>
                    </Tooltip>
                </div>
            </div>
            ...
            <Tabs value={activeTab} onValueChange={handleTabChange} className="space-y-4">
                <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
                    <TabsList className="relative p-0 h-auto rounded-lg flex">
                        {[
                            { value: 'all', label: 'All', count: teamMemberCounts.all },
                            { value: 'published', label: 'Published', count: teamMemberCounts.published },
                            { value: 'hidden', label: 'Hidden', count: teamMemberCounts.hidden },
                        ].map((tab) => (
                            <TabsTrigger
                                key={tab.value}
                                value={tab.value}
