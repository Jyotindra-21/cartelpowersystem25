...
export function DeleteDialog<TData extends IVisitorList>({ ids, children, onSuccess, isBulk = false, table }: DeleteDialogProps<TData>) {
  ...
  async function handleDelete() {
    ...
    try {
      ...
      toast({
        title: 'Error',
        description: isBulk
          ? 'Failed to delete some visitors. Please try again.'
          : 'Failed to delete visitor. Please try again.',
        variant: 'destructive',
      })
    } finally {
      setIsDeleting(false)
  ...
  return (
    <Dialog open={open} onOpenChange={setOpen}>
      ...
      <DialogContent>
        <DialogHeader>
          <DialogTitle>
            {isBulk ? `Delete ${ids.length} Visitors` : 'Delete Visitor'}
          </DialogTitle>
        </DialogHeader>
        <div className="space-y-4">
          <p>
            {isBulk
              ? `Are you sure you want to delete ${ids.length} selected visitors? This action cannot be undone.`
